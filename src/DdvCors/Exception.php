...
class DdvCors
{
  ...
  public static function run($config)
  {
    ...
    if (!$originPass) {
      throw new Exception\Cors("No origin is allowed", 'NO_ORIGIN_ALLOWED');
    }
    if ($method!=='OPTIONS') {
      return true;
    }
    if (!in_array($originMethod, $methods)) {
      throw new Exception\Cors("No method is allowed", 'NO_METHODS_ALLOWED');
    }
    ...
    for ($i=0; $i < $originHeadersLen; $i++) {
      ...
      if(!self::checkHeader($t, $allowHeaders)){
        throw new Exception\Cors('No '.$t.' header is allowed', 'NO_HEADER_ALLOWED');
      }
